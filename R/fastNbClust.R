...
fastNbClust <-function(data = NULL, diss=NULL, distance ="euclidean", min.nc=2, max.nc=15, method =NULL, index = "all", alphaBeale = 0.1)
{
...
if(!is.null(diss))
{
  if((distanceM==1)||(distanceM==2)|| (distanceM==3)|| (distanceM==4)|| (distanceM==5)|| (distanceM==6))
    ...
    if (method == 1)
    {
        hc<-fastcluster::hclust(md,method = "ward.D2")
    }
    if (method == 2)
    {
        hc<-fastcluster::hclust(md,method = "single")
	  }
    if (method == 3)
     {
        hc<-fastcluster::hclust(md,method = "complete")
     }

    if (method == 4)
    {
        hc<-fastcluster::hclust(md,method = "average")
    }

    if (method == 5)
    {
        hc<-fastcluster::hclust(md,method = "mcquitty")

   	}
    if (method == 6)
    {
        hc<-fastcluster::hclust(md,method = "median")

	  }
    if (method == 7)
    {
        hc<-fastcluster::hclust(md,method = "centroid")

	  }
    if (method == 9)
    {
      hc<-fastcluster::hclust(md,method = "ward.D")
        ...
        for (bb in (1:B)) {
            ...
            if (bb == 1) {
                ...
                { set.seed(1)
                  ...
                  pp2 <- cutree(fastcluster::hclust(dist(Xnew), method = method),
                    ClassNr)
